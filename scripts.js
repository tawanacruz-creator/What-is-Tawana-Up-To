...
function showAdminForms() {
    ...
    document.querySelectorAll('.editor-access').forEach(btn => {
...
function initializeEditor() {
    ...
    if (newBtn) newBtn.addEventListener('click', newEditorFile);
    if (saveBtn) saveBtn.addEventListener('click', saveEditorFile);
    if (deleteBtn) deleteBtn.addEventListener('click', deleteEditorFile);

    loadEditorFiles();
}

function getEditorFiles() {
    return JSON.parse(localStorage.getItem('adminEditorFiles') || '[]');
}

function setEditorFiles(files) {
    localStorage.setItem('adminEditorFiles', JSON.stringify(files));
}
...
function loadEditorFiles() {
    ...
    if (files.length === 0) {
        list.innerHTML = '<div class="file-item">No saved files yet</div>';
    ...
    files.forEach(file => {
        ...
        button.addEventListener('click', () => selectEditorFile(file.name));
        if (file.name === currentEditorFile) {
...
function selectEditorFile(name) {
    const files = getEditorFiles();
    const file = files.find(item => item.name === name);
    if (!file) return;

    currentEditorFile = file.name;
...
function newEditorFile() {
    const fileNameInput = document.getElementById('editor-file-name');
    const name = fileNameInput.value.trim() || `untitled-${Date.now()}.txt`;
...
function saveEditorFile() {
    ...
    const files = getEditorFiles();
    const existingIndex = files.findIndex(item => item.name === name);
    const fileData = {
        name,
        content,
...
function deleteEditorFile() {
    ...
    const name = fileNameInput.value.trim();
    if (!name) {
        alert('Enter the file name to delete.');
        return;
    }

    let files = getEditorFiles();
    files = files.filter(item => item.name !== name);
    setEditorFiles(files);
    if (currentEditorFile === name) {
        currentEditorFile = null;
        editor.setValue('', -1);
        fileNameInput.value = '';
    }
...
function initializeSongOfTheDay() {
    ...
    if (songForm) {
        songForm.addEventListener('submit', handleSongUpdate);
    }
}
...
async function handleSongSearch(e) {
    ...
    try {
        ...
        // Use the direct Spotify Open Search API which returns JSON
        const encodedQuery = encodeURIComponent(query);
        const apiUrl = `https://open.spotify.com/opensearch/v1/search?query=${encodedQuery}&types=track&limit=10`;
        ...
        if (response.ok) {
            ...
            if (data.tracks && data.tracks.length > 0) {
                displaySearchResultsFromSpotify(data.tracks);
            } else {
        ...
        resultsContainer.innerHTML = '<div class="search-result-item">Search unavailable. Try entering a Spotify URL instead.</div>';
    }
}

function displaySearchResultsFromSpotify(tracks) {
    const resultsContainer = document.getElementById('search-results');
    ...
    resultsContainer.innerHTML = tracks.map(track => {
        ...
        const artistName = track.artist && track.artist.length > 0 ? track.artist[0] : 'Unknown Artist';
        return `
            <div class="search-result-item" onclick="selectTrack('${trackId}', '${track.name.replace(/'/g, "\\'")}', '${artistName.replace(/'/g, "\\'")}')">
...
function selectTrack(trackId, trackName, artistName) {
    ...
    selectedTrackData = { trackId, trackName, artistName };
    ...
    const updateBtn = document.getElementById('song-update-btn');
    
    selectedSongName.textContent = `${trackName} by ${artistName}`;
...
function handleSongUpdate(e) {
    ...
    document.getElementById('song-update-btn').disabled = true;
}
...
function loadSongOfTheDay() {
    ...
    if (songData) {
        ...
        if (display) {
            display.innerHTML = '<p>No song selected yet. Come back later!</p>';
        }
    }
}
